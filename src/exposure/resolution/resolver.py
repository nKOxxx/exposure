...
def _derive_confidence(s: SignalSet) -> float:
    ...
    penalty = 0.0
    if s.location_conflict:
        penalty += policy.P_LOCATION_CONFLICT
    if s.topic_conflict:
        penalty += policy.P_TOPIC_CONFLICT
...
def _derive_state(s: SignalSet) -> MatchState:
    ...
    # A page that is about somebody else, or that places this person somewhere
    # the subject has never been, cannot be auto-confirmed on soft evidence.
    contradicted = s.location_conflict or s.topic_conflict

    # Strong distinctive direct identifier: personal email/phone/owned domain.
    # These survive a topic conflict — the subject's own email on a page about
    # someone else is still the subject's email — but not a location conflict.
    if s.has_strong_direct:
    ...
    if s.has_username and (s.has_name or corroborators >= 1):
        return MatchState.AMBIGUOUS if contradicted else MatchState.HIGH_CONFIDENCE
    ...
    if s.has_name and corroborators == 1:
        return MatchState.AMBIGUOUS if contradicted else MatchState.POSSIBLE
